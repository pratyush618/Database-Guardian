...
func RestorePostgres(host, username, password, dbname, filePath string, port int) error {
	customLog.Infof("Restoring PostgreSQL database %s from file %s", dbname, filePath)

	// Determine the file extension
	ext := filepath.Ext(filePath)
	if ext == ".gz" {
		return utils.RestoreGzFile(filePath, host, username, dbname, password, port)
	}
